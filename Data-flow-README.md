...
2. کپی فایل سرویس به systemd:
   ...
   sudo cp log_sender.service /etc/systemd/system/
    ...
    1. در ادامه  سرویس rsyslog است که لاگ‌ها را از پورت UDPدریافت می‌کند و لاگ‌های دریافتی را هم در فایل محلی ذخیره می‌کند و هم به Kafka ارسال می‌کند.
...
2. اجرای دستور زیر برای راه‌اندازی سرویس FTP:
   ...
   docker-compose up -d
    ...
    1. در ادامه سرویس Kafka  در حالت KRaft و ابزار مدیریت AKHQ با استفاده از Docker Compose راه اندازی شده است. Kafka یک سیستم پیام‌رسان توزیع شده است و AKHQ یک رابط کاربری وب برای مدیریت و مشاهده کلاستر Kafka می‌باشد.
